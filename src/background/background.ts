...
import { MODELS, getRequiredModelIds } from "../shared/constants.ts";
import { getLLMSettings } from "../shared/llmSettings.ts";
...
import Agent, { resetTextGenerationPipeline } from "./agent/Agent.ts";
import {
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
      ...
      .then((settings) => {
        const requiredModelIds =
          settings.provider === "openai-compatible"
            ...
            : getRequiredModelIds(settings.inBrowserModelId);
  ...
  if (message.type === BackgroundTasks.LLM_SETTINGS_UPDATED) {
    resetTextGenerationPipeline();
    currentAgent?.clear();
    currentAgent = null;
    sendResponse({ status: ResponseStatus.SUCCESS });
    chrome.runtime.sendMessage({
...
// Fix 4: Debounce vector history additions to avoid excessive feature extraction
const tabHistoryQueue = new Map<number, { timeout?: ReturnType<typeof setTimeout>; entry?: { title: string; description: string; url: string } }>();
const VECTOR_HISTORY_DEBOUNCE_MS = 3_000; // 3 second debounce per tab

const addCurrentPageToVectorHistoryDebounced = async (tabId: number, tab: Tab) => {
  const existing = tabHistoryQueue.get(tabId);
  if (existing?.timeout) {
    clearTimeout(existing.timeout);
  }

  tabHistoryQueue.set(tabId, {
    timeout: setTimeout(async () => {
      const queued = tabHistoryQueue.get(tabId);
      tabHistoryQueue.delete(tabId);
      if (queued?.entry) {
        try {
          await vectorHistory.addEntry(queued.entry.title, queued.entry.description, queued.entry.url);
        } catch (error) {
          console.error("Failed to add page to vector history:", error);
        }
      }
    }, VECTOR_HISTORY_DEBOUNCE_MS),
  });
  ...
  // Update the pending entry (will be processed when debounce fires)
  const queued = tabHistoryQueue.get(tabId);
  if (queued) {
    queued.entry = { title, description, url: tab.url };
  }
};
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  // Add page to vector history for later retrieval (debounced)
  addCurrentPageToVectorHistoryDebounced(tabId, tab);
});
